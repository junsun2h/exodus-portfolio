...
> |---|---|
> | 경로 | `img/shatter.gif` |
> | 내용 | 몬스터 1마리 사망 → 메쉬가 삼각형 단위로 분해되어 흩어지는 순간 |
> | 권장 | 720x405 · 12~15fps · 3~5초 · 8MB 이하 (GitHub은 그 이상도 표시되지만 모바일에서 느려진다) |
>
> 부위 그룹화가 보이도록 **정면 또는 살짝 위에서**, 파쇄 직전 0.5초를 포함해 잡으면
> "메쉬가 통째로 사라지는 것이 아니라 부위 단위로 쪼개진다"는 점이 드러난다.
