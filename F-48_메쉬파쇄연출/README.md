...
![인게임 — 몬스터 수십 마리가 동시에 파쇄된다](img/shatter_ingame.webp)

<sub><b>실제 전투에서의 모습이다.</b> 메인 스테이지 104층, 화면을 가득 채운 몬스터가 <b>동시에</b> 죽으면서
전부 조각으로 흩어진다. 아래 단독 데모와 같은 연출이 <b>한 마리가 아니라 수십 마리에 동시에</b> 걸린 것이고,
이 상태에서도 드로우콜이 늘지 않는 것이 이 시스템의 요점이다.
같은 화면에 떠 있는 데미지 숫자(45.43억)와 골드 드랍, 좌하단 콤보 카운터까지가 한 프레임 안에서 처리된다.
960×539 · 51프레임 · 4.1초 (원본 36MB GIF 를 3.0MB 애니메이션 WebP 로 재인코딩).</sub>
