...
- **면접 포인트**: **"AI가 못 하는 일이 아니라, AI가 볼 수 없는 상태가 문제다."** 프리팹 포맷을 AI가 못 읽는다면 AI를 탓할 게 아니라 **읽을 수 있는 표현을 만들어 주면 된다.** 이 판단이 Phase 3의 UGUI→UXML 변환 자동화([F-32](../F-32_UGUItoUXML변환스킬/))와 headless UXML 렌더 캡처 도구로 직결된다. AI 협업을 "프롬프트 잘 쓰기"가 아니라 **인터페이스 설계 문제**로 다룬 첫 사례.
