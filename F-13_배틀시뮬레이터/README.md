...
- **슬라이드 자료**: 시뮬레이터 에디터 창 — **아래 캡처 사용** / "일반 방식 vs 기존 API 호출 방식" 대비 다이어그램 — **다이어그램 필요**
...
<table>
<tr>
<td width="50%" valign="top"><img src="img/simulator-window.webp" alt="배틀 시뮬레이터 메인 창"></td>
<td width="50%" valign="top"><img src="img/damage-breakdown.webp" alt="피해 계산 단계별 브레이크다운"></td>
</tr>
<tr>
<td valign="top"><b>입력</b> — 장비 8부위 × (~전설 / 신화), 스킬·룬 소켓, 오라, 펫을 조합해 빌드를 만들고 프리셋으로 저장한다. <code>최적 장비 찾기</code> / <code>최적 룬 찾기</code>는 <a href="../F-15_최적장비룬탐색기/">F-15</a> 의 탐색기를 이 창에서 바로 돌리는 버튼이다.</td>
<td valign="top"><b>출력</b> — 최종 DPS 하나가 아니라 <b>거기까지 간 모든 단계</b>가 나온다.</td>
</tr>
</table>

<sub><b>이 툴의 값어치는 왼쪽이 아니라 오른쪽에 있다.</b> 밸런싱에서 실제로 막히는 순간은 "DPS가 얼마인가"가
아니라 <b>"왜 이 숫자인가"</b>이고, 그걸 모르면 어떤 값을 만져야 할지 알 수 없다.<br>
오른쪽 캡처를 보면 <code>Skill Effectiveness 326.752%</code> 옆에 <b>그 값이 나온 근거</b>(기본 강화도 65 →
적용된 강화도 MOD 없음 → 최종 강화 레벨 65)가 붙어 있고, <code>Total Base Damage 90,916,549</code> 옆에는
<b>계산식이 숫자를 대입한 상태로</b> 적혀 있다 — <code>(27,824,364) × 326.752% = 90,916,549</code>.
<code>캐스팅 속도 218.61%</code>를 펼치면 <b>기여 MOD 목록</b>이 나오고(<code>mod_castspeed_inc</code> 190.750%),
그 아래 <b>출처 상세 10 items</b> 가 <b>그 MOD를 준 장비·룬·오라가 각각 무엇이었는지</b>까지 되짚어 준다.<br>
즉 <b>최종 DPS에서 개별 장비 옵션 한 줄까지 역추적</b>이 된다. <a href="../F-02_MOD전투수식엔진/">F-02</a> 의
MOD 엔진이 합산 과정을 구조로 들고 있기 때문에 가능한 화면이다.</sub>
